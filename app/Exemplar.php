...
class Exemplar extends Model
{
    ...
    */
     ...
     * @var array
     */
    protected $fillable = ['estante', 'prateleira', 'status', 'danificado', 'reservado', 'livro_id', 'tipoexemplar_id'];
    ...
    public function setExemplaresParaReservado($exemplaresIds) {
        $this->setExemplaresReservado($exemplaresIds);
    }
    
    public function setExemplaresParaNaoReservado($exemplaresIds) {
        $this->setExemplaresReservado($exemplaresIds, 0);
    }
    
    public function setExemplaresReservado($exemplaresIds, $reservado = 1) {
        foreach ($exemplaresIds as $exemplarId) {
            $e = self::findOrFail($exemplarId);
            $e->update(['reservado' => $reservado]);
        }
    }
